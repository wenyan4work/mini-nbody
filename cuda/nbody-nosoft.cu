...
#define SOFTENING  0;//1e-9f

typedef struct { float4 *pos, *vel; } BodySystem;

void randomizeBodies(float *data, int n) {
  for (int i = 0; i < n; i++) {
    data[i] = 2.0f * drand48() - 1.0f;
  }
}
...
//   if(distSqr>0)
     float   invDist = rsqrt(distSqr);
        // cast invDist to zero without if/else by bitwise operation:
        unsigned int exponent = __float_as_uint(invDist)&0x7f800000;
        const unsigned int eight = 8;
        int nnz = ( __popc(exponent) - eight ) ; // =0 if invDist is error, <0 if invDist is normal
        invDist = __uint_as_float( __float_as_uint(invDist) & (nnz >> 31) ); // right shift 32 bits to get 00000 if nnz=0, or 11111 if nnz<0
